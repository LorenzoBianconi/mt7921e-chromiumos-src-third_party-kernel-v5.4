...
static int dramc_probe(struct platform_device *pdev)
{
	...
	pr_info("freq_list: ");
	for (i = 0; i < dramc_dev_ptr->freq_cnt; ++i)
		pr_info(" %d", dramc_dev_ptr->freq_step[i]);
	pr_info("\n");
...
void mtk_dramc_dcm_disable(void)
{
    unsigned int i;
    volatile unsigned int value;
	struct dramc_dev_t *dramc =
		(struct dramc_dev_t *)platform_get_drvdata(dramc_pdev);

    for (i = 0; i < dramc->ch_cnt; i++) {
        value = readl(dramc->ddrphy_chn_base_ao[i] + DDRPHY_AO_MISC_CG_CTRL0);
		// save dcm status
		if (i < MAX_CH_NUM)
        	dramc_dcm_enable[i] = !((value >> DRAMC_CG_SHIFT) & 0x1);
        writel(value | (1 << DRAMC_CG_SHIFT), dramc->ddrphy_chn_base_ao[i] + DDRPHY_AO_MISC_CG_CTRL0);
    }
}
EXPORT_SYMBOL(mtk_dramc_dcm_disable);

void mtk_dramc_dcm_restore(void)
{
    unsigned int i;
    volatile unsigned int value;
	struct dramc_dev_t *dramc =
    ...
    for (i = 0; i < dramc->ch_cnt; i++) {
        if (i < MAX_CH_NUM && dramc_dcm_enable[i]) {
            value = readl(dramc->ddrphy_chn_base_ao[i] + DDRPHY_AO_MISC_CG_CTRL0);
            writel(value & ~(1U << DRAMC_CG_SHIFT),
				dramc->ddrphy_chn_base_ao[i] + DDRPHY_AO_MISC_CG_CTRL0);
        }
    }
}
EXPORT_SYMBOL(mtk_dramc_dcm_restore);
